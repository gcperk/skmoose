...
# filter elevations above threshold value
eval_threshold = 1500

trim <- terra::rast(file.path(out_dir, "dem.tif"))
high_elev <- terra::clamp(trim, lower=eval_threshold, upper=Inf, values=FALSE)
high_elev_poly <- as.polygons(high_elev)


high_elev_sf <- st_as_sf(high_elev_poly) %>%
  st_transform(3005) %>%
...
slope_thresh = 55
  ...
  steep <- clamp(rslope, lower=slope_thresh, upper=Inf, values=FALSE)
  steep_poly <- as.polygons(steep)

  # conver to sf and output
  steep_sf <- st_as_sf(steep_poly)%>%
    st_transform(3005) %>% 
    st_union %>% 
    st_cast("MULTIPOLYGON")
...
# get deciduos leading species

vri <- st_read(file.path(out_dir, "vri.gpkg"))

  vri1 <- vri %>% dplyr::filter(SPECIES_CD_1 %in% c("AT", "AC","EP","SB")) %>%
    dplyr::select(SPECIES_CD_1)

  vri2 <- vri %>% dplyr::filter(SPECIES_CD_2 %in% c("AT", "AC","EP","SB")) %>%
    dplyr::select(SPECIES_CD_2)

  vri3 <- vri %>% dplyr::filter(SPECIES_CD_3 %in% c("AT", "AC","EP","SB")) %>%
    dplyr::select(SPECIES_CD_3)

  vri4 <- vri %>% dplyr::filter(SPECIES_CD_4 %in% c("AT", "AC","EP","SB")) %>%
    dplyr::select(SPECIES_CD_4)

  vri5 <- vri %>% dplyr::filter(SPECIES_CD_5 %in% c("AT", "AC","EP","SB")) %>%
    dplyr::select(SPECIES_CD_5)

  vri6 <- vri %>% dplyr::filter(SPECIES_CD_6 %in% c("AT", "AC","EP","SB")) %>%
    dplyr::select(SPECIES_CD_6)


  vri_moose <- st_join(vri1, vri2)
  vri_moose <- st_join(vri_moose, vri3)
  vri_moose <- st_join(vri_moose, vri4)
  vri_moose <- st_join(vri_moose, vri5)
  vri_moose <- st_join(vri_moose, vri6)

  unique(vri$SPECIES_CD_1)

  vri_moose <- st_union(vri_moose)

  st_write(vri_moose, file.path(out_dir, "vri_decid.gpkg") , append = FALSE)
