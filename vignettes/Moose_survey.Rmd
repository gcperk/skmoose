...
Firstly we need to check the size of the AOI fow download


```{r}

bcdata::bcdc_check_geom_size(aoi)




```
In this case the aoi is too large to perform spatial and needs to be divided into 4 


```{r}
nos <- length(blockno)/4

# approximately 68 blocks

aoi1 <- aoi %>% 
  filter( bkname %in% (c(1:20)))

bcdata::bcdc_check_geom_size(aoi1)
aoitest<- aoi %>% filter(bkname == 2)
...
basedata <- foreach(x = blockno[23:26]) %dopar% {
...
habitable <- foreach(x = blockno[c(1:11)]) %dopar% {
...
out_table <- calculate_areas(blockno[1:11], aoi, out_dir)

write.csv(out_table, file.path(out_dir, "moose_stratification_outputs.csv"), row.names = FALSE)
...
# Note you can adjust low, med and high values within the funtion

out <- assign_categories(out_table, low = 0.4, med = 0.7, high = 1)
