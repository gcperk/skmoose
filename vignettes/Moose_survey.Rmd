...
uninhab <- foreach(x = blockno[42]) %dopar% {
...
habitable <- foreach(x = blockno[42]) %dopar% {
...
burn_strat <- foreach(x = blockno[1:5]) %dopar% {
...
write.csv(out_table, file.path(out_dir, "moose_stratification_outputs_final.csv"), row.names = FALSE)
