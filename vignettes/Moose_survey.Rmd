...
basedata <- foreach(x = blockno[11:20]) %dopar% {
...
uninhab <- foreach(x = blockno[1:20]) %dopar% {
...
uninhab <- foreach(x = blockno[11:20]) %dopar% {
...
habitable <- foreach(x = blockno[1:20]) %dopar% {
...
habitable <- foreach(x = blockno[c(1:20)]) %dopar% {
...
out_table <- calculate_areas(blockno[1:20], aoi, out_dir)
...
```{r}
  library(ggplot2)
library(stats)
#I am still trying to figure out what breaks were used in 2019, it looks #like for the 2018 BVLD survey they were arbitrarily selected at 0-40% =L, #41-70%= Moderate, 71-100%, 71-100% = High. And then the blocks were #adjusted based on past survey data and expert opinion but no clear rules. I #like your idea for the distribution of values if we can try that out to get #round 1 of stratification. 
...
qts <- quantile(out$prop_habit_net_habit_m2)
...
out <- out %>% 
  mutate(class = case_when(
    prop_habit_net_habit_m2 < low ~ "low", 
    prop_habit_net_habit_m2 > high ~ "high", 
    prop_habit_net_habit_m2 > low & prop_habit_net_habit_m2 < high ~ "med"
  ))
