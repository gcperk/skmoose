...
```{r}
uninhab <- foreach(x = blockno[1:10]) %dopar% {
...
habitable <- foreach(x = blockno[1:10]) %dopar% {
...
habitable <- foreach(x = blockno[c(1:10)]) %dopar% {
...
area_tab <- foreach(x = blockno[1:10], .combine=rbind) %dopar% {
