...
basedata <- foreach(x = blockno[51:60]) %dopar% {
...
uninhab <- foreach(x = blockno[51:60]) %dopar% {
...
uninhab <- foreach(x = blockno[51:60]) %dopar% {
  ...
  if(uninhab_sf %in% list.files(temp_out_dir)){
    ...
    unh <-  st_sf(st_sfc(), crs = 3005)
   
      
    if(file.exists(file.path(temp_out_dir,"high_elevation.gpkg"))){
    high_elev <- st_read(file.path(temp_out_dir,"high_elevation.gpkg"))
            unh <- st_union( high_elev_sf, unh)
      
    }
       
    if(file.exists(file.path(temp_out_dir,"rockice.gpkg"))){
    
      rock_ice<- st_read(file.path(temp_out_dir,"rockice.gpkg"))
      
      unh <- st_union( rockice, unh)
    }
    
     
    if(file.exists(file.path(temp_out_dir,"steep.gpkg"))){
      
      steep<- st_read(file.path(temp_out_dir,"steep.gpkg"))
      
      unh <- st_union(steep, unh)
    }
      
    
     if(file.exists(file.path(temp_out_dir,"largelakes.gpkg"))){
      
      largelakes<- st_read(file.path(temp_out_dir,"largelakes.gpkg"))
      
      unh <- st_union(largelakes, unh)
   
       }
 
     # crop to the study area 
      unhab_all = st_intersection(unh,  tmp_aoi)
